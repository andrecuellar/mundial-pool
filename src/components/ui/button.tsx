...
function Button({
  ...
  // When rendering as a Link (or any non-button child) via asChild, the HTML
  // `disabled` attribute is meaningless — `<a>` ignores it and a click still
  // navigates. We close the gap by intercepting the click and adding
  // `aria-disabled` so screen readers + CSS see the state.
  const asChildDisabled = asChild && disabled
  const handleClick = asChildDisabled
  ...
  return (
    <Comp
      ...
      aria-disabled={asChildDisabled ? true : undefined}
      tabIndex={asChildDisabled ? -1 : undefined}
      onClick={handleClick}
      {...props}
    />
  )
